...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  //
  // We sync all .md files from workspace root (not just whitelisted ones)
  // This allows Alfred to create arbitrary files like PLAN.md, NOTES.md, etc.
  // Using find + rsync to sync only files in the root (not subdirs like memory/)
  const syncMdFiles = `find /root/clawd -maxdepth 1 -name '*.md' -type f -exec rsync --no-times {} ${R2_MOUNT_PATH}/workspace/ \\; 2>/dev/null || true`;

  const syncCmd = `rsync -r --no-times --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && mkdir -p ${R2_MOUNT_PATH}/workspace ${R2_MOUNT_PATH}/workspace/memory ${R2_MOUNT_PATH}/workspace/tov ${R2_MOUNT_PATH}/workspace/assets && ${syncMdFiles} && rsync -r --no-times /root/clawd/memory/ ${R2_MOUNT_PATH}/workspace/memory/ 2>/dev/null || true && rsync -r --no-times /root/clawd/tov/ ${R2_MOUNT_PATH}/workspace/tov/ 2>/dev/null || true && rsync -r --no-times /root/clawd/assets/ ${R2_MOUNT_PATH}/workspace/assets/ 2>/dev/null || true && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
